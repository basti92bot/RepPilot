...
const KEY="reppilot-history-v11",WORKING_WEIGHT_KEY="reppilot-working-weights-v1",REST=120,REPS=10,$=id=>document.getElementById(id);
...
const workingWeights=()=>{try{const x=JSON.parse(localStorage.getItem(WORKING_WEIGHT_KEY)||"{}");return x&&typeof x==="object"&&!Array.isArray(x)?x:{}}catch{return{}}};
function rememberWorkingWeight(name,weight){const value=n(weight);if(!name||value<=0)return;const values=workingWeights();values[name]={weight:value,at:new Date().toISOString()};localStorage.setItem(WORKING_WEIGHT_KEY,JSON.stringify(values))}
function preferredWorkingWeight(name,previous){const saved=workingWeights()[name],savedWeight=n(saved?.weight);if(!savedWeight)return previous.weight;const savedAt=Date.parse(saved?.at||"")||0,historyAt=Date.parse(previous.date||"")||0;return savedAt>historyAt?savedWeight:previous.weight}
function updateCurrentWorkingWeight(){if(!active||phase!=="set"||active.id?.startsWith("home-"))return;const input=$("weightInput"),value=n(input?.value);if(!input||input.value.trim()===""||value<=0)return;const e=current();for(let i=si;i<e.sets.length;i++)if(!e.sets[i].done)e.sets[i].weight=value;rememberWorkingWeight(e.name,value)}
window.RepPilotWorkingWeights={key:WORKING_WEIGHT_KEY,remember:rememberWorkingWeight,preferred:preferredWorkingWeight};
...
function start(id){const w=WORKOUTS.find(x=>x.id===id);active={id:w.id,title:w.title,startedAt:new Date().toISOString(),exercises:w.exercises.map(([name,c,def])=>{const p=last(name,def),weight=preferredWorkingWeight(name,p);return{name,lastTraining:p.date?p:null,sets:Array.from({length:c},(_,i)=>({index:i+1,weight,reps:REPS,done:false}))}})};ei=0;si=0;phase="set";cancelRest();renderWorkout();show("workout")}
...
function renderSet(){const e=current(),s=e.sets[si];$("exerciseName").textContent=e.name;$("exerciseTip").textContent=TIPS[e.name]||"Ruhig und kontrolliert ausführen.";$("setCounter").textContent=`Satz ${si+1} von ${e.sets.length}`;$("fixedReps").textContent=e.name.includes("Maximum")?"Saubere Wiederholungen":"10 Wiederholungen";$("lastTraining").hidden=!e.lastTraining;if(e.lastTraining){$("lastWeightValue").textContent=`${kg(e.lastTraining.weight)} kg`;$("lastWeightDate").textContent=`vom ${d(e.lastTraining.date)}`}$("weightInput").value=s.weight;$("previousSet").hidden=si===0;if(si>0){const p=e.sets[si-1];$("previousSet").textContent=`Letzter Satz: ${kg(p.weight)} kg × ${p.reps}`}$("completeSetBtn").textContent=si===e.sets.length-1?"Letzten Satz abschließen":"Satz abschließen"}
function completeSet(){const e=current(),s=e.sets[si];if($("weightInput").value.trim()==="")return $("weightInput").focus();s.weight=n($("weightInput").value);rememberWorkingWeight(e.name,s.weight);s.done=true;lastSet={name:e.name,no:si+1,weight:s.weight};if(si<e.sets.length-1){e.sets[si+1].weight=s.weight;beginRest({type:"set",index:si+1})}else beginRest({type:"complete"})}

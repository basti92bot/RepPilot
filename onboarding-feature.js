(() => {
  ...
  const FOCUS={
    strength:{label:"Krafttraining",copy:"Dein Wochenplan besteht aus Krafttraining im Studio."},
    running:{label:"Laufen",copy:"Intervall-, Tempo- und lockere Läufe passend zur Häufigkeit."},
    mixed:{label:"Kraft + Laufen",copy:"RepPilot kombiniert Krafttraining und Lauftraining in einer Woche."}
  };
  const DAYS=[{id:1,label:"Mo",name:"Montag"},{id:2,label:"Di",name:"Dienstag"},{id:3,label:"Mi",name:"Mittwoch"},{id:4,label:"Do",name:"Donnerstag"},{id:5,label:"Fr",name:"Freitag"},{id:6,label:"Sa",name:"Samstag"},{id:0,label:"So",name:"Sonntag"}];
  ...
  const validDays=v=>Array.isArray(v)?DAYS.map(x=>x.id).filter(d=>v.map(Number).includes(d)):[];

  const stepFor=name=>{
    const n=String(name||"").toLowerCase();
    if(/beinpresse|rumänisches kreuzheben|wadenheben/.test(n))return 5;
    if(/seitheben|curl|fly|kabel|extension|pushdown|reverse butterfly|trizeps/.test(n))return 1;
    return 2.5;
  };
  ...
  function ensureStyles(){
    ...
    s.textContent=`
      ...
      .rp-option-title{display:block;margin:18px 0 8px;font-size:12px;font-weight:900;color:#475569}.rp-options{display:grid;gap:8px}.rp-options.two{grid-template-columns:1fr 1fr}.rp-options.three{grid-template-columns:repeat(3,1fr)}.rp-option{width:100%;text-align:left;background:#fff;color:#111827;border:2px solid #e2e8f0;border-radius:14px;padding:12px 13px}.rp-option strong,.rp-option small{display:block}.rp-option small{margin-top:3px;color:#64748b;line-height:1.3}.rp-option.selected{border-color:#111827;background:#f8fafc}.rp-option.selected strong:after{content:" ✓"}
      .rp-frequency .rp-option{text-align:center;padding:13px 8px}.rp-day-grid{display:grid;grid-template-columns:repeat(7,1fr);gap:5px}.rp-day{padding:11px 2px;text-align:center;border:2px solid #e2e8f0;border-radius:11px;background:#fff;color:#111827;font-weight:900}.rp-day.selected{background:#111827;color:#fff;border-color:#111827}.rp-day-count{display:block;margin-top:7px;color:#64748b;font-size:12px}
      #rpOnboardingSave{width:100%;margin-top:18px;border:0;border-radius:14px;padding:14px;background:#111827;color:#fff;font:inherit;font-weight:900}.rp-onboarding-note{margin-top:10px!important;font-size:12px;color:#64748b!important}
      ...
      @media(max-width:390px){#rpOnboarding{padding-left:12px;padding-right:12px}.rp-onboarding-card{padding:18px}.rp-onboarding-card h2{font-size:24px}.rp-onboarding-grid{gap:8px}.rp-option{padding:10px}.rp-day{font-size:11px}}
    `;
    document.head.appendChild(s);
  }

  function selectOption(root,selector,datasetKey,value){root.querySelectorAll(selector).forEach(x=>x.classList.toggle("selected",x.dataset[datasetKey]===String(value)));}
  function selectedDays(o){return DAYS.map(x=>x.id).filter(d=>o.querySelector(`[data-rp-day="${d}"]`)?.classList.contains("selected"));}
  function updateDayCount(o){const wanted=Number(o.dataset.frequency||0),count=selectedDays(o).length,el=o.querySelector("#rpDayCount");if(el)el.textContent=wanted?`${count} von ${wanted} Trainingstagen gewählt`:"Wähle zuerst 2, 3 oder 4 Trainingstage pro Woche.";}

  function ensureOverlay(){
    let o=document.getElementById("rpOnboarding");if(o)return o;
    ensureStyles();
    o=document.createElement("div");o.id="rpOnboarding";o.hidden=true;
    o.innerHTML=`<div class="rp-onboarding-card"><small>REPPILOT START</small><h2 id="rpOnboardingTitle">Dein persönlicher Trainingsplan 🏋️</h2><p id="rpOnboardingIntro">Gib kurz deine Daten und Trainingswünsche an. RepPilot erstellt daraus deinen Wochenplan und passende Startgewichte.</p><div class="rp-onboarding-grid"><div class="rp-onboarding-field"><label for="rpOnboardingHeight">Größe</label><div class="rp-onboarding-input"><input id="rpOnboardingHeight" type="number" min="100" max="250" step="1" inputmode="numeric"><span>cm</span></div></div><div class="rp-onboarding-field"><label for="rpOnboardingWeight">Gewicht</label><div class="rp-onboarding-input"><input id="rpOnboardingWeight" type="number" min="30" max="300" step="0.1" inputmode="decimal"><span>kg</span></div></div></div><span class="rp-option-title">Geschlecht</span><div class="rp-options two">${Object.entries(SEXES).map(([id,x])=>`<button type="button" class="rp-option" data-rp-sex="${id}"><strong>${x.label}</strong></button>`).join("")}</div><span class="rp-option-title">Wie erfahren bist du?</span><div class="rp-options">${Object.entries(LEVELS).map(([id,x])=>`<button type="button" class="rp-option" data-rp-level="${id}"><strong>${x.label}</strong><small>${x.copy}</small></button>`).join("")}</div><span class="rp-option-title">Was möchtest du trainieren?</span><div class="rp-options">${Object.entries(FOCUS).map(([id,x])=>`<button type="button" class="rp-option" data-rp-focus="${id}"><strong>${x.label}</strong><small>${x.copy}</small></button>`).join("")}</div><span class="rp-option-title">Wie oft möchtest du trainieren?</span><div class="rp-options three rp-frequency">${[2,3,4].map(n=>`<button type="button" class="rp-option" data-rp-frequency="${n}"><strong>${n} Tage</strong></button>`).join("")}</div><span class="rp-option-title">An welchen Wochentagen?</span><div class="rp-day-grid">${DAYS.map(x=>`<button type="button" class="rp-day" data-rp-day="${x.id}" title="${x.name}">${x.label}</button>`).join("")}</div><span id="rpDayCount" class="rp-day-count">Wähle zuerst 2, 3 oder 4 Trainingstage pro Woche.</span><button id="rpOnboardingSave" type="button">Plan erstellen & RepPilot starten</button><p class="rp-onboarding-note">Einsteiger erhalten vorsichtige Startgewichte und die erste Kraftmessung nach 28 Tagen. Fortgeschrittene und Profis messen bei Kraftübungen bereits im ersten Training. Die Messung kann übersprungen werden.</p></div>`;
    ...
    o.querySelectorAll("[data-rp-focus]").forEach(b=>b.onclick=()=>{o.dataset.focus=b.dataset.rpFocus;selectOption(o,"[data-rp-focus]","rpFocus",b.dataset.rpFocus);});
    o.querySelectorAll("[data-rp-frequency]").forEach(b=>b.onclick=()=>{o.dataset.frequency=b.dataset.rpFrequency;selectOption(o,"[data-rp-frequency]","rpFrequency",b.dataset.rpFrequency);const wanted=Number(b.dataset.rpFrequency),chosen=selectedDays(o);if(chosen.length>wanted)chosen.slice(wanted).forEach(d=>o.querySelector(`[data-rp-day="${d}"]`)?.classList.remove("selected"));updateDayCount(o);});
    o.querySelectorAll("[data-rp-day]").forEach(b=>b.onclick=()=>{const wanted=Number(o.dataset.frequency||0);if(!wanted){o.querySelector("[data-rp-frequency]")?.focus();return;}const selected=b.classList.contains("selected");if(!selected&&selectedDays(o).length>=wanted)return;b.classList.toggle("selected");updateDayCount(o);});
  ...
  async function saveOnboarding(){
    const o=ensureOverlay(),heightCm=Number(document.getElementById("rpOnboardingHeight")?.value||0),weightKg=Number(document.getElementById("rpOnboardingWeight")?.value||0),sex=o.dataset.sex,trainingLevel=o.dataset.level,trainingFocus=o.dataset.focus,trainingDaysPerWeek=Number(o.dataset.frequency||0),trainingDays=selectedDays(o);
    ...
    if(!FOCUS[trainingFocus]){o.querySelector("[data-rp-focus]")?.focus();return;}
    if(![2,3,4].includes(trainingDaysPerWeek)){o.querySelector("[data-rp-frequency]")?.focus();return;}
    if(trainingDays.length!==trainingDaysPerWeek){o.querySelector("#rpDayCount").textContent=`Bitte genau ${trainingDaysPerWeek} Tage auswählen.`;return;}
    const old=window.repPilotProfile?.get?.()||{},at=old.onboardingCompletedAt||new Date().toISOString(),profile={heightCm,weightKg,sex,trainingLevel,trainingFocus,trainingDaysPerWeek,trainingDays,onboardingCompletedAt:at};

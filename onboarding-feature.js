  ...
  function seedStrengthCycle(profile,at){
    try{
      ...
      (Array.isArray(WORKOUTS)?WORKOUTS:[]).forEach(w=>(w.exercises||[]).forEach(([name,,base])=>{
        ...
        rows.push({date:at,exercise:name,mode:"onboarding",trainingWeight:suggestWeight(name,base,profile),formula:"Onboarding-Startwert aus Körpergewicht + Level + Geschlecht; Krafttest nach 28 Tagen"});
      }));

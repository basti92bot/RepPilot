...
const HOME_WEEK=[
{day:1,dayName:"Montag",title:"Home Workout A",type:"strength",workoutId:"home-a",meta:"Ganzkörper · ca. 35–45 Min."},
{day:2,dayName:"Dienstag",title:"Erholung",type:"rest",meta:"Spaziergang oder Mobility"},
{day:3,dayName:"Mittwoch",title:"Home Workout B",type:"strength",workoutId:"home-b",meta:"Ganzkörper · ca. 35–45 Min."},
{day:4,dayName:"Donnerstag",title:"Lockerer Dauerlauf",type:"run",runId:"easy",meta:"Ruhiges Gesprächstempo"},
{day:5,dayName:"Freitag",title:"Home Workout C",type:"strength",workoutId:"home-c",meta:"Ganzkörper · ca. 35–45 Min."},
{day:6,dayName:"Samstag",title:"Erholung",type:"rest",meta:"Spaziergang oder Mobility"},
...
function ensureCustomWorkouts(){
  try{
    ...
    add({id:"home-a",day:1,dayName:"Montag",title:"Home Workout A",exercises:[
      ["Kniebeugen",3,0],["Liegestütze bis Maximum",3,0],["Ausfallschritte",3,0],["Rudern mit Band",3,0],["Schulterdrücken mit Band",2,0],["Glute Bridge",3,0],["Crunches",3,0]
    ]});
    add({id:"home-b",day:3,dayName:"Mittwoch",title:"Home Workout B",exercises:[
      ["Bulgarian Split Squats",3,0],["Enge Liegestütze",3,0],["Rumänisches Kreuzheben mit Rucksack",3,0],["Bandrudern",3,0],["Seitheben mit Band",2,0],["Hip Thrust",3,0],["Crunches",3,0]
    ]});
    add({id:"home-c",day:5,dayName:"Freitag",title:"Home Workout C",exercises:[
      ["Kniebeugen mit Rucksack",3,0],["Liegestütze bis Maximum",3,0],["Einarmiges Rudern mit Rucksack",3,0],["Pike Push-ups",2,0],["Reverse Lunges",3,0],["Bizeps Curls mit Band",2,0],["Trizepsdrücken mit Band",2,0],["Crunches",3,0]
    ]});
    add({id:"loss-a",day:1,dayName:"Montag",title:"Ganzkörper Kraft A",exercises:[
      ["Beinpresse",3,80],["Brustpresse",3,40],["Latzug neutral",3,45],["Schulterpresse",2,25],["Rumänisches Kreuzheben",3,50],["Seil-Pushdown",2,20],["Incline Curls",2,10],["Crunch-Maschine",3,25]
    ]});
    add({id:"loss-b",day:5,dayName:"Freitag",title:"Ganzkörper Kraft B",exercises:[
      ["Beinbeuger",3,35],["Schrägbankdrücken leicht",3,40],["Brustgestütztes Rudern",3,40],["Beinstrecker",3,35],["Seitheben",2,6],["Hammercurls",2,10],["Cross Body Cable Extension",2,8],["Wadenheben",3,50],["Crunch-Maschine",3,25]
    ]});
...
function selectedWeek(){const id=read();return id==="home"?HOME_WEEK:id==="weightloss"?WEIGHTLOSS_WEEK:null}
function dayIcon(x){return x.type==="run"?"🏃":x.type==="rest"?"😴":"🏋️"}
function renderCustomWeek(week){
  ...
  if(icon)icon.textContent=dayIcon(t);if(label)label.textContent=`Heute ist ${t.dayName}`;if(title)title.textContent=t.title;if(meta)meta.textContent=t.meta;if(hint)hint.textContent=t.type==="run"?"Geschwindigkeit und Laufdaten direkt in RepPilot.":t.type==="rest"?"Heute ist Regeneration eingeplant.":"Dein ausgewählter Trainingsplan für heute.";
  if(btn){if(t.workoutId){btn.hidden=false;btn.textContent="Training starten";btn.onclick=()=>start(t.workoutId)}else if(t.runId){btn.hidden=false;btn.textContent="Laufplan anzeigen";btn.onclick=()=>openRun(t.runId)}else btn.hidden=true}
  if(plan){plan.innerHTML=week.slice().sort((a,b)=>((a.day+6)%7)-((b.day+6)%7)).map(x=>`<article class="plan-item ${x.day===new Date().getDay()?"today":""}"><div class="plan-icon">${dayIcon(x)}</div><div class="plan-copy"><div class="day">${x.dayName}</div><h3>${x.title}</h3><p>${x.meta}</p></div>${x.workoutId?`<button data-selected-workout="${x.workoutId}">Starten</button>`:x.runId?`<button data-selected-run="${x.runId}">Plan</button>`:`<span class="badge">Erholung</span>`}</article>`).join('');plan.querySelectorAll('[data-selected-workout]').forEach(b=>b.onclick=()=>start(b.dataset.selectedWorkout));plan.querySelectorAll('[data-selected-run]').forEach(b=>b.onclick=()=>openRun(b.dataset.selectedRun))}
}
...
function renderSelectedHome(){if(baseRenderHome)baseRenderHome();activePlanCard();const week=selectedWeek();if(week)renderCustomWeek(week)}
if(baseRenderHome){renderHome=function(){renderSelectedHome()}}

function init(){styles();ensureCustomWorkouts();renderSelectedHome();profile();const v=document.querySelector('header h1 span');if(v)v.textContent='v11.8.18 CLOUD TEST';document.title='RepPilot v11.8.18';let n=0;const timer=setInterval(()=>{n++;if(profile()||n>20)clearInterval(timer)},250)}

...
const inventory=JSON.parse(fs.readFileSync("exercise-image-manifest.json","utf8"));
...
async function verify(){
  ...
  if(inventory.assets.length!==43)failures.push("Erwartet: 43 aktive Bilddateien");
  if(failures.length)throw new Error(failures.join("; "));
}
let last="";
for(let attempt=1;attempt<=12;attempt++){
  try{await verify();console.log("Live-Deployment-Test bestanden: "+expected+", alle 43 Bilder mit exakter Prüfsumme.");process.exit(0);}

...
try {
  ...
  check(swState.script.includes("sw.js?v=11.8.112"),"Aktiver Service Worker hat aktuelle Version",swState.script);
  check(swState.keys.includes("reppilot-v11-8-111"),"Aktueller PWA-Cache vorhanden",swState.keys.join(","));
  check(swState.requests.some(x=>x.includes("icon-192.png?v=11.8.112")),"192er Icon im Runtime-Cache");
  check(swState.requests.some(x=>x.includes("icon-512.png?v=11.8.112")),"512er Icon im Runtime-Cache");

  // Manifest runtime fetch
  const manifestRuntime=await page.evaluate(async()=>{
    const r=await fetch("./manifest.json?v=11.8.112",{cache:"no-store"});
  ...
  await page.evaluate(async()=>{
    ...
    const reg=await navigator.serviceWorker.register("./sw.js?v=11.8.112&reinstall=1",{updateViaCache:"none"});

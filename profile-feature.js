(() => {
  ...
  async function loadCloudProfile(){
    ...
    if(!c||!u)return local;
    const{data,error}=await c.from("profiles").select("height_cm,weight_kg,training_level,onboarding_completed_at").eq("id",u.id).maybeSingle();
  ...
  async function saveCloudProfile(p){
    ...
    const payload={id:u.id,height_cm:p.heightCm||null,weight_kg:p.weightKg||null,training_level:p.trainingLevel||null,onboarding_completed_at:p.onboardingCompletedAt||null};
  ...
  function renderProfile(p=readLocal()){
    ...
    if(p.heightCm&&p.weightKg){
      s.innerHTML=`<div class="profile-values"><div><strong>${Number(p.weightKg).toLocaleString("de-DE",{maximumFractionDigits:1})} kg</strong><small>Gewicht</small></div><div><strong>${Number(p.heightCm).toLocaleString("de-DE")} cm</strong><small>Größe</small></div><div><strong>${LEVEL_LABELS[p.trainingLevel]||"–"}</strong><small>Level</small></div><div><strong>${(Number(p.weightKg)*.5).toLocaleString("de-DE",{maximumFractionDigits:1})} kg</strong><small>Hanging Leg Raises</small></div></div>`;
    }else s.innerHTML=`<p class="muted">Trage Größe, Körpergewicht und Trainingslevel ein. RepPilot kann damit Startgewichte und Körpergewichtsübungen besser anpassen.</p>`;
  ...
  function ensureStyles(){
    ...
    s.textContent=`#profileLevel{width:100%;box-sizing:border-box;border:2px solid #d1d5db;border-radius:16px;padding:15px 14px;background:#fff;color:var(--text);font:inherit;font-weight:800;margin-bottom:12px}`;
  ...
  function ensureProfileUI(){
    ...
    document.getElementById("editProfileBtn").onclick=async()=>{const p=await loadCloudProfile();document.getElementById("profileHeight").value=p.heightCm||"";document.getElementById("profileWeight").value=p.weightKg||"";document.getElementById("profileLevel").value=p.trainingLevel||"advanced";document.getElementById("profileForm").hidden=false};
    document.getElementById("saveProfileBtn").onclick=async()=>{
      const heightCm=Number(document.getElementById("profileHeight").value),weightKg=Number(document.getElementById("profileWeight").value),trainingLevel=document.getElementById("profileLevel").value;
      if(heightCm<100||heightCm>250||weightKg<30||weightKg>300||!LEVEL_LABELS[trainingLevel])return;
      const old=readLocal();
      await saveProfile({heightCm,weightKg,trainingLevel,onboardingCompletedAt:old.onboardingCompletedAt||new Date().toISOString()});
